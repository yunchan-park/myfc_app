...
class _AddMatchStep4ScreenState extends State<AddMatchStep4Screen> {
  ...
  Future<void> _submitMatch() async {
    ...
    try {
      // StorageService와 AuthService 모두에서 확인
      final teamIdFromStorage = await _storageService.getTeamId();
      final tokenFromStorage = await _storageService.getToken();
      final teamIdFromAuth = await _authService.getTeamId();
      final tokenFromAuth = await _authService.getToken();
      
      // AuthService를 우선 사용하고, 없으면 StorageService 사용 (타입 변환 처리)
      String? teamId = teamIdFromAuth?.toString() ?? teamIdFromStorage;
      String? token = tokenFromAuth ?? tokenFromStorage;
