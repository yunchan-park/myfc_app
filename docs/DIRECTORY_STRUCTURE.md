...
│       └── helpers.dart             # 헬퍼 함수
├── test/                            # 테스트 코드
│   └── widget_test.dart             # Widget 및 단위 테스트
├── assets/                          # 앱 에셋
│   ├── images/                      # 이미지 파일
...
└── workflows/                   # 워크플로우 정의
    ├── flutter.yml             # Flutter CI/CD
    └── fastapi.yml              # FastAPI CI/CD
```
...
- `frontend/run_tests.dart` - 프론트엔드 테스트
- `backend/run_tests.py` - 백엔드 테스트
...
# 미사용 코드 탐지 (자동화)
dart run_tests.dart --analyze     # Dead code detection
```
...
# 개별 실행
cd frontend && flutter test       # Frontend only
cd backend && python run_tests.py # Backend only
```
...
**이 구조는 현대적인 풀스택 애플리케이션의 베스트 프랙티스를 반영하여 설계되었으며, 지속적인 최적화를 통해 축구 클럽 관리의 모든 요구사항을 효율적으로 처리할 수 있습니다.**